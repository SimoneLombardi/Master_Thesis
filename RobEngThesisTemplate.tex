...
\newcommand{\codetxt}[2]{{\fontfamily{qcr}\selectfont\textcolor{#1}{#2}}}
...
\ifpdf
    \pdfinfo {/Title (Example of M.Sc. Thesis with RoboticsLaTeX template)
              /Creator (TeX)
              /Producer (pdfTeX)
              /Author (Barbara Bruno, Fulvio Mastrogiovanni, Enrico Simetti)
...
\title{Development and analysis of a control architecture for a hyper-redundant articulated robot}

\ifpdf
  \author{\myhlink{mailto:name@unige.it}{Simone Lombardi}}
  ...
  \supervisors{
  	  \begin{tabular}{rl}
  		Professor &  \myhlink{giorgio.cannata@unige.it}{Giorgio Cannata} \\
  		PhD. & \myhlink{francesco.grella@edu.unige.it}{Francesco Grella}  \\
  		PhD. & \myhlink{francesco.giovinazzo@edu.unige.it}{Francesco Giovinazzo} \\
  		PhD. & \myhlink{simone.borelli@edu.unige.it}{Simone Borelli} \\
  	\end{tabular}
  	}
  \crest{\includegraphics[width=50mm]{logo_unige}}
\else
